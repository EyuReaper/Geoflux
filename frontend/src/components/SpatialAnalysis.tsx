          ...
          <div className="grid grid-cols-3 gap-1 p-1 bg-white/5 rounded-xl border border-white/10">
            {tools.map(tool => (
              <button 
                ...
                className={cn(
                  "flex flex-col items-center gap-1 py-2 rounded-lg text-[9px] font-bold uppercase transition-all",
                  spatialAggregationConfig.type === tool.id ? "bg-orange-500 text-black" : "text-white/40 hover:text-white"
                )}
              >
                <tool.icon size={14} />
                {tool.label}
